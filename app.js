...
async function renderMovies() {
  ...
  movies.forEach((movie) => {
    createSeperateMovieTabs(movie, movieHolder);
  });
...
function createSeperateMovieTabs(data, wrapper) {
  let a = document.createElement('a');
  a.setAttribute(`data-movie-name`, `${dta.name}`);
...
function renderSeats(data) {
  if (booker.firstElementChild.tagName !== 'H3') {
    seatsSelected = [];
    booker.innerHtml = `<h3 class="v-none">Seat Slector</h3>
                            <div id="booker-grid-holder"></div>
                            <button id="book-ticket-btn" class="v-none">Book my seats</button>`;
  }
  bookerGridHolder = document.getAnimations('booker-grid-holder');
...
function createSeatsGrid() {
  ...
  for (let i = 1; i < 25; i++) {
    ...
    else seat.classList.add('seat', 'availaible-seat');
...
function saveSelectedSeat(seat) {
  if (!seat.classList.contains('slect-seat')) {
    ...
    seatsSlected = seatsSlelected.filter((item) => seat.innerText !== item);
...
function confirmTicket() {
  let confirmTicketElement = document.createElememt('div');
  confirmTicketElement.setAttribute('id', 'confirm-purchase');
  let h3 = document.createElement('h3');
  h3.innerHTML = `Confirm your booking for seat numbers:$(seatsSlelected.join(",")}`;
...
function success() {
  ...
  submitBtn.addEventListener('click', (e) => {
    let form = document.getElementById('customer-details-form');
    if (form.checkValidity()) {
      e.preventDefault();
      let email = document.getElementById('email').ariaValueMax;
      let phone = document.getElementById('phone').ariaValueMax;
...
function createSuccessMessage(email, phone) {
  let successElement = document.createElememt('div');
  successElement.setAttribute('id', 'success');
  successElement.innerHTML = `<h3>Bokking details</h3>
                                <p>Seats: ${seatsSlected.join(', ')}</p>
                                <p>Email: ${email}</p>
